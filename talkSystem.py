...
def main():
    ...
    try:
        ...
        LLM = OpenAILLM()
        ryuseiAI = CharacterAI(LLM, "龍星", contextDB_json, 13, 1.2,fifoPlayer,TachieViewer, 'ryusei')
        metanAI = CharacterAI(LLM, "めたん", contextDB_json, 6, 1.2,fifoPlayer,TachieViewer, 'metan')
        tumugiAI = CharacterAI(LLM, "つむぎ", contextDB_json, 8, 1.2,fifoPlayer,TachieViewer, 'tumugi')
        zundamonAI = CharacterAI(LLM, "ずんだもん", contextDB_json, 7, 1.2,fifoPlayer,TachieViewer, 'zundamon')
        ...
        for i in range(100):
            try:
                conversationTimingChecker.check_conversation_timing_with_delay(fifoPlayer.get_file_queue_length)
